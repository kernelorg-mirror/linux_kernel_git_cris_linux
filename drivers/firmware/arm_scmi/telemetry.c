...
struct scmi_telemetry_update_notify_payld {
	...
	__le32 status;
	__le32 num_dwords;
	__le32 array[] __counted_by_le(num_dwords);
};
...
struct scmi_msg_resp_telemetry_shmti_list {
	__le32 num_shmti;
	struct scmi_shmti_desc desc[] __counted_by_le(num_shmti);
};
...
struct scmi_msg_resp_telemetry_reading_complete {
	__le32 num_dwords;
	__le32 dwords[] __counted_by_le(num_dwords);
};
				      ...
				      const void *response, void *priv)
{
	...
	p->next = (void *)r->desc;
	p->rx_len = st->rx_len - sizeof(*r);

	return 0;
}

static inline size_t scmi_telemetry_tde_var_size_calc(struct telemetry_de *tde)
{
	size_t sz = 0;

	sz += tde->ts_type == TSTAMP_LINE ? sizeof(__le32) : 0;
	sz += tde->de.fc_support ? sizeof(struct de_desc_fc) : 0;
	sz += tde->de.name_support ? SCMI_SHORT_NAME_MAX_SIZE : 0;
					      ...
					      void **next, size_t *rx_len)
{
	...
	*next += sizeof(*desc);
	...
	payld_sz += scmi_telemetry_tde_var_size_calc(tde);
	if (*rx_len < payld_sz)
		return -ENOSPC;
					...
					unsigned int *active_update_interval)
{
	...
	/* Spec mandates the ALL selector when per-group config not available */
	if (!ti->info.per_group_config_support)
		flags = TELEMETRY_GET_SELECTOR_ALL;
	else
		flags = grp_id == SCMI_TLM_GRP_INVALID ?
			TELEMETRY_GET_SELECTOR_ORPHANS :
			TELEMETRY_GET_SELECTOR_GROUP;
				   ...
				   struct scmi_iterator_state *st, void *priv)
{
	...
	/*
	 * Guessing that, if a DE is reported as enabled with timestamp, it has
	 * to support timestamp: NOTE THAT the timestamp type (tde->ts_type) can
	 * ONLY be discovered later by  DE_DESCRIPTION.
	 */
	tde->de.tstamp_support = tde->de.tstamp_enabled;
...
static int scmi_telemetry_initial_state_lookup(struct telemetry_info *ti)
{
	...
	for (int id = 0; id < ti->num_shmti; id++) {
		ret = scmi_telemetry_shmti_scan(ti, id, SCAN_UPDATE);
		if (ret)
			dev_warn(dev,
...
scmi_telemetry_de_groups_init(struct device *dev, struct telemetry_info *ti)
{
	...
	if (num_groups < ti->info.base.num_groups) {
		for (int i = 0; i < num_groups; i++) {
			...
			rinfo->grps[i].des = NULL;
			kfree(rinfo->grps[i].des_str);
			rinfo->grps[i].des_str = NULL;
		}
				       ...
				       void *priv)
{
	...
	if (!IS_ALIGNED(phys_addr, 4)) {
		dev_err(ph->dev,
			"SHMTI ID %u is NOT 32-bit aligned (paddr:0x%llX). Abort.\n",
	...
	if (!IS_ALIGNED(phys_addr, 8))
		dev_warn(ph->dev,
			 "SHMTI ID:%u is NOT 64bit aligned (paddr:0x%llX)\n",
			 shmti->info.sid, phys_addr);
...
static void scmi_telemetry_group_disable(struct telemetry_info *ti,
					 struct scmi_telemetry_group *grp)
{
	struct scmi_telemetry_res_info *rinfo;
	 ...
	 * and invalidate SHMTI reference.
	 */
	for (int i = 0; i < grp->info->num_des; i++) {
...
static inline struct payload __iomem *
scmi_telemetry_de_payld_safe_calc(struct telemetry_shmti *shmti, u32 de_offs)
{
	struct payload __iomem *payld;
	u64 de_base, de_payld;
	u32 de_end;

	/* Check if accessing payld->meta is safe. */
	if (check_add_overflow(de_offs, sizeof(u32), &de_end) ||
	    de_end > shmti->info.len)
		return IOMEM_ERR_PTR(-EPROTO);

	de_base = (__force u64)shmti->base;
	if (check_add_overflow(de_base, de_offs, &de_payld))
		return IOMEM_ERR_PTR(-EPROTO);

	payld = shmti->base + de_offs;
	/* Check line boundary */
	if (check_add_overflow(de_offs, LINE_LENGTH_BYTES(payld), &de_end) ||
	    de_end > shmti->info.len)
		return IOMEM_ERR_PTR(-EPROTO);
...
static int
scmi_telemetry_tde_linked_lines_process(struct telemetry_info *ti,
					struct telemetry_de *tde,
					struct telemetry_shmti *shmti,
					struct payload __iomem *payld,
					u32 bts_offs)
{
	...
	/* Process related BLK_TS lines if any */
	guard(mutex)(&tde->mtx);
	...
	if (bts_payld) {
		if (scmi_telemetry_shmti_overflow(shmti, bts_payld)) {
			trace_scmi_tlm_access(0, "SHMTI_OVERFLOW", 0, 0);
			return -EINVAL;
		}
	} else {
		dev_warn_once(ti->ph->dev,
			      "Received BLK_TS with zero offset.\n");
		trace_scmi_tlm_access(0, "BLKTS_WITH_ZERO_OFFSET", 0, 0);
	}

	/* Look for a viable BLK_TS line */
	tde->bts = scmi_telemetry_blkts_bind(ti, shmti,
					     payld, bts_payld);
	if (WARN_ON(!tde->bts)) {
		trace_scmi_tlm_access(tde->de.info->id, "INVALID_USE_BLK_TS", 0, 0);
		return -EPROTO;
	}

	/* Update per-DE BLK_TS if any */
	if (tde->ts_type == TSTAMP_BLK &&
	    tde->bts->last_rate)
...
static int
scmi_telemetry_state_set_tde_resp(struct telemetry_info *ti,
				  struct scmi_telemetry_de *de,
				  u32 sid, u32 de_offs, u32 bts_offs)
{
	struct telemetry_shmti *shmti;
	struct payload __iomem *payld;
	struct telemetry_de *tde;

	/* A FastChannel DE MUST return an INVALID SHMTI ID on enable */
	if (WARN_ON(de->fc_support)) {
		trace_scmi_tlm_access(de->info->id, "INVALID_DE_FC_ON_SHMTI", 0, 0);
		return -EPROTO;
	}

	/* Update SHMTI/offset while skipping non-SHMTI based DES */
	shmti = &ti->shmti[sid];
	/* Check sanity of received location... */
	payld = scmi_telemetry_de_payld_safe_calc(shmti, de_offs);
	if (IS_ERR(payld))
		return PTR_ERR(payld);

	tde = to_tde(de);
	scoped_guard(mutex, &tde->mtx) {
		/* ...then assign once validated */
		tde->sid = sid;
		tde->base = shmti->base;
		tde->offset = de_offs;
	...
	return scmi_telemetry_tde_linked_lines_process(ti, tde, shmti,
						       payld, bts_offs);
}
				      ...
				      void *r, bool is_group)
{
	...
	if (IS_SHMTI_ID_VALID(sid)) {
		...
		if (!is_group) {
			...
			scmi_telemetry_group_disable(ti, obj);
		}
	}
			       ...
			       struct scmi_telemetry_de_sample *sample)
{
	...
	sample->tstamp = tde->ts_type == TSTAMP_LINE ? LINE_TSTAMP_GET(fc) : 0;
...
scmi_telemetry_msg_payld_process(struct telemetry_info *ti,
				 unsigned int num_dwords, __le32 *dwords,
				 size_t payld_bytes)
{
	struct scmi_telemetry_res_info *rinfo;
	u32 next = 0;
	...
	while (next < num_dwords) {
		...
		if (LINE_TYPE_RAW(meta) != TDCF_DATA_LINE) {
			trace_scmi_tlm_access(de_id, "MSG_BAD_TYPE", 0, 0);
			continue;
		}

		if (USE_BLK_TS_RAW(meta)) {
			trace_scmi_tlm_access(de_id, "MSG_BAD_USE_BLKTS", 0, 0);
			continue;
		}
					 ...
					 struct scmi_telemetry_de_sample *samples)
{
	...
	if (!ret) {
		...
		u32 num_dwords = le32_to_cpu(r->num_dwords);

		/* Update cached DEs values from payload */
		if (num_dwords) {
			size_t payld_sz = 0, dwords_sz;

			if (check_mul_overflow(num_dwords, sizeof(r->dwords[0]), &dwords_sz) ||
			    check_add_overflow(sizeof(*r), dwords_sz, &payld_sz) ||
			    payld_sz > t->rx.len) {
				dev_err(ph->dev,
					"Malformed Telemetry delayed response - num_dwords:%u  payld_sz:%zd  rx_len:%zd\n",
					num_dwords, payld_sz, t->rx.len);
				ret = -EPROTO;
				goto out;
			}

			scmi_telemetry_msg_payld_process(ti, num_dwords, r->dwords,
							 payld_sz - sizeof(*r));
		}
...
static void scmi_telemetry_local_resources_reset(struct telemetry_info *ti)
{
	...
	for (int i = 0; i < rinfo->num_des; i++) {
		...
		/*
		 * Unlinking ONLY BLK_TS lines since, per the SCMI spec, the
		 * UUID/DE associations are permanent within the same session
		 * boot: after a reset has cleared the server-side configuration
		 * the next time SHMTIs will be re-populated, these same UUID/DE
		 * associations will be re-instated.
		 */
		scmi_telemetry_blkts_unlink(tde);
	}
...
static int scmi_telemetry_reset(const struct scmi_protocol_handle *ph)
{
	...
	if (!ret) {
		...
		/* NO need to lookup the initial state we have just reset all */
	}
...
__scmi_telemetry_resources_get(struct telemetry_info *ti)
{
	/* Ensure rinfo descriptor is visible */
	return smp_load_acquire(&ti->rinfo);
}

static void scmi_telemetry_event_cleanup(struct telemetry_info *ti)
{
	guard(mutex)(&ti->events_mtx);
...
static void scmi_telemetry_resources_free(void *arg)
{
	...
	/* Cleanup any still registered event */
	scmi_telemetry_event_cleanup(ti);
	...
	dev_dbg(ti->ph->dev, "SCMI Telemetry resources freed for instance\n");
}
...
scmi_telemetry_resources_enumerate(struct telemetry_info *ti)
{
	...
	/*
	 * Ensure the following initialization can be called only once
	 * from one thread of execution.
	 */
	if (atomic_cmpxchg(&ti->rinfo_initializing, 0, 1)) {
		/*
		 * When initialization is already ongoing in another thread,
		 * just wait for its completion and return the fully or partially
		 * populated rinfo.
		 */
	...
	/* Note that this code below can be run only once by one thread */

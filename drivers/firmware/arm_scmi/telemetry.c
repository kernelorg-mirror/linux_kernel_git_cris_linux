				      ...
				      void *r, bool is_group)
{
	...
	if (IS_SHMTI_ID_VALID(sid)) {
		...
		if (!is_group) {
			...
			payld = ti->shmti[sid].base;
			/* Check boundary first... */
			if (de_offs + LINE_LENGTH_BYTES(payld) >=
			    ti->shmti[sid].info.len)
				return -EPROTO;
				 ...
				 size_t payld_bytes)
{
	...
	while (next < num_dwords) {
		...
		wlen = LINE_LENGTH_WORDS_RAW(payld->meta);
		/* Check Boundary */
		if (next + wlen > payld_bytes / sizeof(u32))
			return;

		next += wlen;
		if (DATA_INVALID_RAW(payld->meta)) {
			trace_scmi_tlm_access(payld->id, "MSG_INVALID", 0, 0);
			continue;
		}
